...
export default function LocationContactSection({ settings }: { settings?: LocationSettings | null }) {
  ...
  const subscriptionPhoneNumber = settings?.subscriptionPhoneNumber?.trim() || null;
                ...
                {subscriptionPhoneNumber && (
                  <div className="flex items-center gap-4">
                    ...
                    <div className={`flex-1 ${isRtl ? "text-right" : "text-left"}`}>
                      <a
                        href={`tel:${subscriptionPhoneNumber.replace(/\s/g, "")}`}
                        className="text-drd-primary font-semibold hover:text-drd-primary-dark transition-colors block"
                      >
                        <span dir="ltr" style={{ unicodeBidi: "isolate" }}>{subscriptionPhoneNumber}</span>
                      </a>
                      <span className="text-drd-muted text-xs">
                        {tField(lang, "For subscription plans", "لخطط الاشتراك")}
                      </span>
                    </div>
                  </div>
                )}
